...
switch ($ARGV[0]) {
  case 'start'  { print "Configuring LIO Target LUNs...\n"; }
  case 'stop'   { exit; }
...
my $raid_set_size = 8; #include parity drives
my $expected_drive_count = 45;
...
for ( my $adp = 0; $adp < $adapters; $adp++ ) {
  ...
  for $i (0..(floor($num_drives/$raid_set_size)-1)) {
    ...
    if (!$done_ld) {
      print "running: $MegaCli -CfgLdAdd -r6[$PDLIST] -a$adp -NoLog\n";
      if ( !$opt_d ) {
        system("$MegaCli", "-CfgLdAdd", "-r6[$PDLIST]", "-a$adp", "-NoLog");
        if ( $? == -1 ) {
